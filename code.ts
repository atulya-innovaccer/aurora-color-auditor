...
const SEMANTIC_COLOR_MAP: Record<string, { base: string; semantic: string }> = {
...
// ============================================
// COMPONENT COLOR VALIDATION RULES
// ============================================
...
const DS_COMPONENT_RULES: Record<string, ComponentAppearance[]> = {
    'Badge': [
        // Solid appearances
...
function detectComponentType(nodeName: string): string | undefined {
    const lowerName = nodeName.toLowerCase();
    if (lowerName.includes('badge')) return 'Badge';
    if (lowerName.includes('button') || lowerName.includes('btn')) return 'Button';
    if (lowerName.includes('message') || lowerName.includes('inline-message')) return 'Message';
    if (lowerName.includes('chip') || lowerName.includes('tag')) return 'Chip';
...
function getSemanticRole(hex: string): string | undefined {
    const mapping = SEMANTIC_COLOR_MAP[hex.toUpperCase()];
    return mapping?.semantic;
}

function getTextColorRole(hex: string): string | undefined {
...
interface ContrastResult {
    ...
    // DS Component validation fields
    detectedComponent?: string;
    componentAppearance?: string;
    isValidDSCombination: boolean;
    dsCombinationIssue?: string;
    semanticRole?: string;
...
function processNode(node: SceneNode): ContrastResult | null {
    ...
    let componentAppearance: string | undefined;
    let isValidDSCombination = true;
    let dsCombinationIssue: string | undefined;
...
figma.ui.onmessage = async (msg: { type: string; nodeId?: string; nodeIds?: string[]; width?: number; height?: number }) => {
