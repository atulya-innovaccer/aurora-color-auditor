...
function matchTextScale(fontSize, fontWeight) {
    if (fontSize === null)
        return null;
...
function findNearestDSColor(hex) {
...
function getWcagGuideline(ratio, isLargeText, isText, fontSize) {
...
function calculateContrastFromHex(fgHex, bgHex) {
...
// ============================================
function getTokenName(hex, figmaStyleName) {
    if (figmaStyleName)
...
function findBackgroundColor(node) {
    ...
    while (current !== null) {
        if (current.type === 'PAGE' || current.type === 'DOCUMENT')
            break;
        if ('fills' in current) {
            const fill = extractSolidFill(current);
            if (fill && fill.opacity > 0.1)
                return fill;
        }
        current = current.parent;
    }
...
// SMART SEVERITY
// ============================================
function hasReadableContent(node, containerFill) {
    if (!('children' in node))
        return false;
    for (const child of node.children) {
        ...
        if (child.type === 'TEXT') {
            ...
            if (textFill && textFill.opacity > 0.1 && calculateContrastRatio(textFill, containerFill) >= 4.5)
                return true;
        }
        if (child.type === 'VECTOR' || child.type === 'BOOLEAN_OPERATION') {
            const vectorFill = extractSolidFill(child);
            if (vectorFill && vectorFill.opacity > 0.1 && calculateContrastRatio(vectorFill, containerFill) >= 3.0)
                return true;
        }
        if ('children' in child && hasReadableContent(child, containerFill))
            return true;
    }
    return false;
}
function assessContainerSeverity(node, foreground, background, ratio) {
    if (ratio >= 3.0)
        return { status: 'pass', isDecorative: false };
...
function passesWCAG_AA(ratio, isLarge) {
    return isLarge ? ratio >= 3.0 : ratio >= 4.5;
}
function passesWCAG_AAA(ratio, isLarge) {
...
function processNode(node) {
    ...
    if (hasComplexFill(node)) {
        return {
            ...
            fontSize: node.type === 'TEXT' ? getFontSize(node) : null,
    ...
    if (!foreground || foreground.opacity < 0.1)
        return null;
    const background = findBackgroundColor(node);
    const contrastRatio = calculateContrastRatio(foreground, background);
    const fontSize = node.type === 'TEXT' ? getFontSize(node) : null;
    const fontWeight = node.type === 'TEXT' ? getFontWeight(node) : 400;
...
function traverseNodes(node, results) {
    ...
    if ('children' in node) {
        for (const child of node.children)
            traverseNodes(child, results);
    }
}
// ============================================
// COLOR MATRIX
// ============================================
function collectUniqueColors(results) {
...
function generateColorMatrix(uniqueColors, sourceName, results) {
    const combinations = [];
    // Build a map of combinations to node IDs
    const comboNodeMap = new Map();
    for (const res of results) {
        if (!res.foreground || !res.background)
            continue;
        const key = `${res.foreground.hex}_${res.background.hex}`;
        if (!comboNodeMap.has(key))
            comboNodeMap.set(key, []);
    ...
    if (msg.type === 'scan') {
        ...
        for (const node of selection) {
            ...
            if ('children' in node) {
                ...
                for (const child of node.children)
                    traverseNodes(child, allResults);
            }
        }
        // ... (rest of scan logic) ...
        ...
        allResults.sort((a, b) => {
            const order = { fail: 0, warning: 1, complex: 2, pass: 3 };
